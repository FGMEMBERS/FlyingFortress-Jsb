...
var pushback = {
	...
	add_fixed_pushback : func() {
		...
		var distance=7.5; # correction for tailwheel x-offset from zero point
...
#### needs to be parametrized and packed
			...
			rootpos.apply_course_distance(course, distance);
...
#### needs to be parametrized and packed
		var course=geo.normdeg(heading+180); #180 deg to get away from the plane
		truckpos.apply_course_distance(course, distance);
print("Model: "~me.model["local"].getPath());
		setprop("voodoomaster/pushback/truck-heading", getprop("gear/gear[2]/steering-norm")*60);

print("planeX="~truckpos.x()~" truckX="~me.pushbacks["local"].root.x()~" dist="~truckpos.distance_to(me.pushbacks["local"].root));
		...
		setprop("voodoomaster/pushback/truck-x-diff", truckpos.distance_to(me.pushbacks["local"].root));
